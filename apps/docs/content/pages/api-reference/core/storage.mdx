...
Generic CRUD contract used by framework helpers. `assertModels` is optional and lets handler boot fail early for enabled storage features without querying the database.
...
The framework reads/writes three built-in models. Users own the real schema and map these logical fields in adapters.
...
The dispatcher consults `isCommandDisabled` automatically when `storageFeatures.disabledCommands` is enabled on `createCommandHandler`.
...
Empty list = no restriction. Non-empty = command runs only in those channels for that guild when `storageFeatures.channelLocks` is enabled.
