...
Enable `storageFeatures.disabledCommands`; then expose admin commands to flip the flag.
...
Enable `storageFeatures.channelLocks` for the dispatcher to consult channel locks alongside the static `channels` field.
